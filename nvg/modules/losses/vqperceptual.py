...
from nvg.modules.losses.lpips import LPIPS
from nvg.modules.discriminator.model import NLayerDiscriminator, weights_init
from nvg.modules.losses.dinodisc import DinoDisc as DINODiscriminator
from nvg.modules.losses.diffaug import DiffAug
...
class VQLPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start, codebook_weight=1.0, pixelloss_weight=1.0,
                 ...
                 commit_weight = 0.25,
                 perceptual_weight=1.0, use_actnorm=False, disc_conditional=False, dino_disc=False,
        ...
        self.disc_type = 'dinodisc' if dino_disc else 'nlayerdisc'
        if dino_disc:
            self.discriminator = DINODiscriminator(norm_type='sbn')  # default 224 otherwise crop
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
                global_step, last_layer=None, cond=None, split="train", latent=None, enc_last_layer=None):
        ...
        if optimizer_idx == 0:
            ...
            if self.disc_type == 'dinodisc':
                logits_fake = self.discriminator(self.daug.aug(reconstructions.contiguous(), 0))
            else:
                if cond is None:
            ...
            loss = nll_loss + g_loss + codebook_loss * self.commit_weight
        ...
        if optimizer_idx == 1:
            ...
            if self.disc_type == 'dinodisc':
                logits_real = self.discriminator(self.daug.aug(inputs.contiguous().detach(), 0))
                logits_fake = self.discriminator(self.daug.aug(reconstructions.contiguous().detach(), 0))
            else:
                if cond is None:
