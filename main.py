...
class WrappedDataset(Dataset):
    ...
    def __init__(self, dataset):
        ...
        num = 4
        for k, v in self.data.data.labels.items():
            self.data.data.labels[k] = v[:num]
        self.data.data._length = num
...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def _train_dataloader(self):
        return DataLoader(self.datasets["train"], batch_size=self.batch_size, num_workers=self.num_workers, shuffle=True, drop_last=True)
...
class ImageLogger(Callback):
    def __init__(
        ...
        # self.log_steps = [2**n for n in range(int(np.log2(self.batch_freq)))]
        self.log_steps = [1]
        ...
        if (
            ...
            with torch.no_grad(), torch.amp.autocast('cuda', **gpu_autocast_kwargs):
                images = pl_module.log_images(
                    batch, max_images=self.max_images, split=split, **self.log_images_kwargs
                )
...
def init_wandb(save_dir, opt, config, group_name, name_str):
    ...
    if opt.debug:
        ...
        config_serializable = {key: value for key, value in config.items() if not callable(value)}
...
class CheckpointEveryEpoch(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        filename = f"epoch_{epoch:03}.ckpt"
...
if __name__ == "__main__":
    ...
    statisticsdir = os.path.join(logdir, "statistics")
    ...
    try:
        model.set_statisticsdir(statisticsdir)
        os.makedirs(statisticsdir, exist_ok=True)
    except:
        pass
    ...
    default_modelckpt_cfg = {
        ...
        "params": {
            ...
            "verbose": True,
            "save_last": True,
            "save_top_k": -1,                  # Save all checkpoints, not just best k
    ...
    seed_everything(opt.seed+trainer.global_rank, workers=True)
    ...
    if not cpu:
        ngpu = int(lightning_config.trainer.devices) * int(
            lightning_config.trainer.num_nodes)
    else:
        ngpu = 1
    ...
    if opt.scale_lr:
        model.learning_rate = accumulate_grad_batches * ngpu * bs * base_lr / 256
        print(
